...
class NN2():
    ...
    def nn2(self, x):
        ...
        y_hat_1 = y_hat_1.reshape(np.shape(y_hat_1)[1],np.shape(y_hat_1)[2])
    ...
    def emp_loss_grad(self, train_X, train_y, W, radnom_W_index):
        ...
        sum_img_emp_loss = [np.zeros((1, np.shape(self.layers[0].W)[0], np.shape(self.layers[0].W)[1])),np.zeros((np.shape(self.layers[1].W)[1],np.shape(self.layers[1].W)[0]))]
        #enter image by image
        a = 0
